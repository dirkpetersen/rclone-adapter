...
class BisyncOptions(BaseModel):
    ...
    no_cleanup: bool = Field(
        False, description="Retain working files (useful for troubleshooting and testing)."
    )
    no_slow_hash: bool = Field(
        False, description="Ignore listing checksums only on backends where they are slow"
    )
    recover: bool = Field(
        False, description="Automatically recover from interruptions without requiring --resync."
    )
    remove_empty_dirs: bool = Field(
        False, description="Remove ALL empty directories at the final cleanup step."
    )
    resilient: bool = Field(
        False,
    ...
    slow_hash_sync_only: bool = Field(
        False,
...
class CopyOptions(BaseModel):
    ...
    combined: bool = Field(
        False, description="string         Make a combined report of changes to this file"
    )
    create_empty_src_dirs: bool = Field(
        False, description="Create empty source dirs on destination after copy"
    )
    csv: bool = Field(False, description="Output in CSV format")
    dest_after: bool = Field(
        False, description="string       Report all files that exist on the dest post-sync"
    )
    differ: bool = Field(
        False, description="string           Report all non-matching files to this file"
    )
    flag: bool = Field(
        False,
    ...
    match: bool = Field(
        False, description="string            Report all matching files to this file"
    )
    missing_on_dst: bool = Field(
        False, description="string   Report all files missing from the destination to this file"
    )
    missing_on_src: bool = Field(
        False, description="string   Report all files missing from the source to this file"
    )
    flag: bool = Field(
        False,
...
class MoveOptions(BaseModel):
    ...
    create_empty_src_dirs: bool = Field(
        False, description="Create empty source dirs on destination after move"
    )
    csv: bool = Field(False, description="Output in CSV format")
...
class SyncOptions(BaseModel):
    ...
    create_empty_src_dirs: bool = Field(
        False, description="Create empty source dirs on destination after sync"
    )
    csv: bool = Field(False, description="Output in CSV format")
    dest_after: bool = Field(
        False, description="string       Report all files that exist on the dest post-sync"
