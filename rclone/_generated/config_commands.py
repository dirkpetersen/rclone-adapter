...
class AuthorizeOptions(BaseModel):
    ...
    auth_no_open_browser: bool = Field(
        False, description="Do not automatically open auth link in default browser"
    )
    template: bool = Field(
        False,
...
class ListremotesOptions(BaseModel):
    ...
    source: bool = Field(
        False, description="string        Filter remotes by source, e.g. 'file' or 'environment'"
    )
