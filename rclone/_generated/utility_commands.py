...
class BackendOptions(BaseModel):
    ...
    flag: bool = Field(
        False, description="--option stringArray   Option in the form name=value or name"
    )
...
class CatOptions(BaseModel):
    ...
    offset: bool = Field(
        False, description="int         Start printing at offset N (or from end if -ve)"
    )
    separator: bool = Field(
        False, description="string   Separator to use between objects when printing multiple files"
    )
...
class CopyurlOptions(BaseModel):
    ...
    header_filename: bool = Field(
        False, description="Get the file name from the Content-Disposition header"
    )
    no_clobber: bool = Field(False, description="Prevent overwriting file with same name")
    print_filename: bool = Field(
        False, description="--print-filename    Print the resulting name from --auto-filename"
    )
...
class DeleteOptions(BaseModel):
    ...
    rmdirs: bool = Field(
        False, description="rmdirs removes empty directories but leaves root intact"
    )
...
class LinkOptions(BaseModel):
    ...
    expire: float | None = Field(
        0.0, description="Duration   The amount of time that the link will be valid (default off)"
    )
...
class RcOptions(BaseModel):
    ...
    arg: bool = Field(
        False, description='--arg stringArray      Argument placed in the "arg" array'
    )
    json: bool = Field(
        False, description="string          Input JSON - use instead of key=value args"
    )
    ...
    password: bool = Field(
        False, description="string          Password to use to connect to rclone remote control"
    )
    ...
    user: bool = Field(
        False, description="string          Username to use to rclone remote control"
    )
...
class SelfupdateOptions(BaseModel):
    ...
    version: bool = Field(
        False, description="string   Install the given rclone version (default: latest)"
    )
...
class TouchOptions(BaseModel):
    ...
    recursive: bool = Field(False, description="--recursive          Recursively touch all files")
