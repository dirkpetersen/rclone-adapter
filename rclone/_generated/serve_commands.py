...
class MountOptions(BaseModel):
    ...
    allow_non_empty: bool = Field(
        False, description="Allow mounting over a non-empty directory (not supported on Windows)"
    )
    allow_other: bool = Field(
        False, description="Allow access to other users (not supported on Windows)"
    )
    allow_root: bool = Field(
        False, description="Allow access to root user (not supported on Windows)"
    )
    async_read: bool = Field(
        False, description="Use asynchronous reads (not supported on Windows) (default true)"
    )
    daemon: bool = Field(
        False,
    ...
    dir_perms: bool = Field(
        False, description="FileMode                     Directory permissions (default 777)"
    )
    direct_io: bool = Field(False, description="Use Direct IO, disables caching of data")
    file_perms: bool = Field(
        False, description="FileMode                    File permissions (default 666)"
    )
    fuse_flag: bool = Field(
        False,
    ...
    link_perms: bool = Field(
        False, description="FileMode                    Link permissions (default 666)"
    )
    mount_case_insensitive: bool = Field(
        False,
    ...
    noappledouble: bool = Field(
        False,
        description="Ignore Apple Double (._) and .DS_Store files (supported on OSX only) (default true)",
    )
    noapplexattr: bool = Field(
        False, description='Ignore all "com.apple.*" extended attributes (supported on OSX only)'
    )
    flag: bool = Field(
        False,
    ...
    vfs_block_norm_dupes: bool = Field(
        False,
    ...
    vfs_cache_mode: bool = Field(
        False,
        description="CacheMode               Cache mode off|minimal|writes|full (default off)",
    )
    vfs_case_insensitive: bool = Field(
        False, description="If a file name not found, find a case insensitive match"
    )
    vfs_fast_fingerprint: bool = Field(
        False, description="Use fast (less accurate) fingerprints for change detection"
    )
    vfs_links: bool = Field(
        False,
        description="Translate symlinks to/from regular files with a '.rclonelink' extension for the VFS",
    )
    vfs_metadata_extension: bool = Field(
        False, description="string          Set the extension to read metadata from"
    )
    vfs_refresh: bool = Field(
        False, description="Refreshes the directory cache recursively in the background on start"
    )
    volname: bool = Field(
        False,
        description="string                         Set the volume name (supported on Windows and OSX only)",
    )
    write_back_cache: bool = Field(
        False,
    ...
    no_modtime: float | None = Field(
        0.0, description="Don't read/write the modification time (can speed things up)"
    )
    ...
    vfs_cache_max_age: float | None = Field(
        0.0,
        description="Duration             Max time since last access of objects in the cache (default 1h0m0s)",
    )
    vfs_cache_max_size: int | None = Field(
        0, description="SizeSuffix          Max total size of objects in the cache (default off)"
    )
    vfs_cache_min_free_space: int | None = Field(
        0,
        description="SizeSuffix    Target minimum free space on the disk containing the cache (default off)",
    )
    vfs_cache_poll_interval: int | None = Field(
        0, description="Duration       Interval to poll the cache for stale objects (default 1m0s)"
    )
    vfs_disk_space_total_size: int | None = Field(
        0, description="SizeSuffix   Specify the total space of disk (default off)"
    )
    vfs_read_ahead: int | None = Field(
        0,
        description="SizeSuffix              Extra read ahead over --buffer-size when using cache-mode full",
    )
    vfs_read_chunk_size: int | None = Field(
        0, description="SizeSuffix         Read the source objects in chunks (default 128Mi)"
    )
    vfs_read_chunk_size_limit: int | None = Field(
        0,
    ...
    vfs_read_chunk_streams: int | None = Field(
        0, description="int             The number of parallel streams to read at once"
    )
    vfs_read_wait: float | None = Field(
        0.0,
        description="Duration                 Time to wait for in-sequence read before seeking (default 20ms)",
    )
    vfs_used_is_size: int | None = Field(
        0, description="rclone size           Use the rclone size algorithm for Used size"
    )
    vfs_write_back: float | None = Field(
        0.0,
        description="Duration                Time to writeback files after last use when using cache (default 5s)",
    )
    vfs_write_wait: float | None = Field(
        0.0,
